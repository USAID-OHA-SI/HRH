...
for (i in 1:length(OU_list)) {
  ...
  topLevel <- topLevel %>% filter(mech_code %in% topLevel2) %>%
    ...
    filter(mech_code %in% DREAMS_mechs_OU)
  ...
  DREAMS_check <- DREAMS_check %>% filter(mech_code %in% overall_DREAMS) %>%
    filter(mech_code %in% DREAMS_mechs_OU)
  overall_long <- overall_long %>% filter(mech_code %in% DREAMS_mechs_OU)
...
# Set workbook title
wbTitle <- paste0("FY24 HRH Data Quality Checks - DREAMS mechanisms only") 
...
wbName <- paste0("./4. Outputs/QC Reports/DREAMS only/FY24 HRH Data Quality Checks - DREAMS mechs only.xlsx")
