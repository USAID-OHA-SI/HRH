...
## Execute a for loop that creates a QC report for each OU
    ...
    # Now, create the for loop that goes through each OU, and then appends into each list
...
for (i in 1:length(OU_list)) {
  ...
  OU <- OU_list[i] # loop through each OU in the OU list
      ...
      topLevel$pct_difference_fromER <- ifelse(is.infinite(topLevel$pct_difference_fromER), 100, topLevel$pct_difference_fromER) # set infinite values to 100
            ...
            # Create a second table that calculates the prime + sub total, append to the prime/sub table, and then sort by mech code
              topLevel_total <- HRH_ER_merged %>%
                filter(year == max(year)) %>%
                filter(operating_unit == OU) %>%
                group_by(year, operating_unit, mech_code, mech_name) %>%
                summarise(ER_expenditure_amt = sum(ER_expenditure_amt[HRH_relevant == "Y"], na.rm = T),
                          HRH_expenditure_amt = sum(HRH_expenditure_amt, na.rm = T)) %>%
                ungroup() %>%
                mutate(prime_or_sub = "Prime+Sub", .before = ER_expenditure_amt) %>% # add the prime/sub column = "All"
                mutate(pct_difference_fromER = (HRH_expenditure_amt - ER_expenditure_amt)/ER_expenditure_amt * 100,
                       action_item = if_else(abs(pct_difference_fromER) > 10, "Reported HRH expenditures are significantly different from ER staffing expenditures by at least 10%. Please review for closer alignment.", ""))
              topLevel_total$pct_difference_fromER[is.nan(topLevel_total$pct_difference_fromER)] <- NA # convert NaN's to NA 
              topLevel_total$pct_difference_fromER <- ifelse(is.infinite(topLevel_total$pct_difference_fromER), 100, topLevel_total$pct_difference_fromER) # set infinite values to 100
              
            # row bind the rows together and sort
              topLevel <- rbind(topLevel_total, topLevel) %>%
                arrange(mech_code)
...
## Now load each iteration into each  long list
  ...
  primeSubList[[i]] <- topLevel
  missingList[[i]] <- totalCount
  aboveSiteList[[i]] <- aboveSite
  otherList[[i]] <- Breakdown_other
}
    
## Now bind the rows together in each list
  overall_long = do.call(rbind, overallList)
  topLevel = do.call(rbind, primeSubList)
  totalCount = do.call(rbind, missingList)
  aboveSite = do.call(rbind, aboveSiteList)
  Breakdown_other = do.call(rbind, otherList)
      ...
      wb <- loadWorkbook("./1. Data/HRH_QC_Reporting_Template_20221118_vF - All Countries.xlsx")
      
      # Set workbook title
      wbTitle <- paste0("All OU's - Data Quality Checks on Submitted HRH Reporting Templates") 
      ...
      wbName <- paste0("./4. Outputs/QC Reports/Deep dives/HRH Data Quality Checks - Deep Dive - All.xlsx")
