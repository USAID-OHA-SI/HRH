...
---
title: "FY23 Human Resources for Health (HRH) Inventory Briefer - `r params$operating_unit`"
---
...
This HRH Inventory Briefer provides a detailed summary of the FY23 HRH Inventory for `r params$operating_unit`. This report will summarize the submitted HRH inventory templates by prime implementation partners to help country teams better monitor and understand their staffing footprint. 
...
## HRH Inventory Completeness and Data Quality
...
## Summary of PEPFAR vs USAID HRH Staffing footprint
...
## Funding Agency
Breakdown of HRH expenditure and number of individual staff supported by USAID, CDC, and other agencies. This provides better understanding of how many staff (and how much they cost) that USAID and CDC recruited to implement their FY23 program activities.  

```{r, echo = FALSE, eval = TRUE}

## Create a Transformer Function that will be used by our dual axis plots. The transformer function will 'transform' the second axis, such that it will be scaled with the first axis for visualization via dual axis plots ----
...
# With the transformer function, we can now start making dual axis plots.
...
# Create a y-label nudge for use in this graph based on the max value of the y-axis
y_axis_nudge <- dual_axis %>%
  summarise(max_y_value = max(actual_annual_spend)) %>%
  mutate(nudge = max_y_value * .00002) %>%
  pull(nudge)
...
g2 <- g1 +
  ...
  # Add formatting of the dual axis plots
...
## Top Employment Titles (based on individual count or based on HRH expenditure)
The following two graphs show the top employment titles based on (i) highest individual count, or (ii) based on highest HRH expenditure for USAID. The first graphs shows the most common employment title being recruited by USAID based on individual count. These two graphs may look different because the most common employment titles with the highest staff count are typically community-based staff that are less expensive, while the more expensive staff are typically non-service delivery staff that provide more program management or above service delivery activities.
...
g2 <- g1 +
  ...
  labs(title = stringr::str_glue("Top 5 Employment Titles (by individual count)")) +
...
g2 <- g1 +
  ...
  labs(title = stringr::str_glue("Top 5 Employment Titles (by HRH expenditure)")) +
...
This scatter plot shows the range of annual remuneration (i.e. annual salary) of the top employment titles in USAID. Each dot is an individual's annual remuneration. This graph generally shows the general spread or range of remuneration for each employment title. 
...
# Now plot the summary df
  ...
  scale_fill_manual(values = c("#808080", "#1e87a5")) + # format the color
    
  scale_fill_discrete(name = "Prime Local Partner?") +
...
## Top Employment Titles for Community-Based Staff (by count)
...
## FY23 Top Primary Beneficiaries for Community-Based Staff
