...
#       aes(x=reorder(employment_title, equivalent_annual_salary, FUN = median), 
#           y=equivalent_annual_salary, 
#           group = employment_title)) + 
#  geom_boxplot() + 
#  labs(x="Employment Title", y = "Estimated Annual Remuneration / Salary") +
#  coord_flip()

### NEXT STEPS: FIX THE BOX PLOT. NEED TO REFINE THE OUTLIER REMOVER FUNCTION. ONLY REMOVE THE TOP 1-2 OUTLIER POINTS THAT ARE SUPER HIGH. 

# AFTER THAT, FIX THE DOT FORMATTING
...
ggplot(local_partner, aes(x = reorder(program, -actual_annual_spend), 
  ...
  scale_fill_manual(values = c("#808080", "#1e87a5")) +
...
## FY23 Staffing Expenditures and Count by Work Location

```{r fig7, fig.height = 5, fig.width = 7, echo = FALSE, eval = TRUE}
...
g1 <- dual_axis %>%
  ...
  labs(title = stringr::str_glue("Top Employment Titles for Community-based Staff (by count)")) +
...
## FY23 Top Primary Beneficiaries
...
g1 <- dual_axis %>%
    ggplot(aes(x = reorder(beneficiary, -individual_count))) +
    geom_col(
        aes(y = individual_count, fill = "Individual Count"),
        alpha = 0.9,
        width = 0.8
...
#scale_y_continuous(labels = scales::comma)
  ...
  labs(title = stringr::str_glue("Individual Count and HRH Expenditure by Primary Beneficiary")) +
...
## FY23 Top Prime Implementatino Partners by count

```{r fig10, fig.height = 5, fig.width = 8, echo = FALSE, eval = TRUE}
