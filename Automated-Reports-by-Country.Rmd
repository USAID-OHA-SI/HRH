...
Table 2 provides a summary of USAID’s staffing size relative to the overall PEPFAR program in `r params$operating_unit`. This includes the total HRH staffing expenditure, individual count, full-time equivalence (FTE), and the percentage of total expenditure going towards HRH. This information serves as an indicator for USAID’s level of staffing investment in the country relative to other implementing agencies’ HRH investments.
...
Figure 1 provides a summary of the total staffing expenditure and total headcount supported by each PEPFAR implementing agency (USAID, CDC, and Other agencies) for implementation of FY24 program activities.
...
```{r fig10, fig.height = 8, fig.width = 10, echo = FALSE, eval = TRUE}
...
g3 <- g2 + g1 + plot_annotation('Figure 3: HRH Expenditure and Individual Count by Primary Program Area', theme=theme(plot.title=element_text(size=15)))
...
## Top Employment Titles Based on HRH Expenditure or Individual Count
...
```{r fig4, fig.height = 7, fig.width = 6, echo = FALSE, eval = TRUE}

# Create a df that shows HRH expenditure by the top 5 employment title
patchwork_expenditure <- HRH_clean %>%
  filter(fiscal_year == 2024) %>%
  filter(funding_agency == "USAID") %>%
  filter(operating_unit == params$operating_unit) %>%
...
# Top graph for top titles by expenditure

g1 <- ggplot(data = patchwork_expenditure, mapping = aes(x = reorder(employment_title_wrapped, -actual_annual_spend), y = actual_annual_spend, fill = employment_title_wrapped)) +
    geom_col(alpha = 0.9,
        width = 0.8) +
  
    scale_fill_manual(values = c("#2057a7", 
                              "#2057a7", 
                              "#2057a7",
                              "#2057a7",
                              "#2057a7",
        ...
        geom_label_repel(data = patchwork_expenditure, aes(label = scales::dollar(round(actual_annual_spend))),
             size = 3,
             position = "identity",
             min.segment.length = 0.25,
             direction = "y",
    ...
    labs(title = stringr::str_glue("Top 5 using HRH Expenditure")) +
    theme(axis.line=element_blank(),
        axis.text.y=element_blank(),
        axis.ticks=element_blank(),
        axis.title.x=element_blank(),
...
# Bottom graph for titles by count

g2 <- ggplot(data = patchwork_count, mapping = aes(x = reorder(employment_title_wrapped, -individual_count), y = individual_count, fill = employment_title_wrapped)) +
    geom_col(alpha = 0.9,
        width = 0.8) +
  
    scale_fill_manual(values = c("#1e87a5", 
                              "#1e87a5", 
                              "#1e87a5",
                              "#1e87a5",
                              "#1e87a5",
    ...
    geom_label_repel(data = patchwork_count, aes(label = scales::comma(round(individual_count))),
             size = 3,
             position = "identity",
             min.segment.length = 0.25,
             direction = "y",
    ...
    labs(title = stringr::str_glue("Top 5 using Individual Count")) +
    theme(axis.line=element_blank(),
        axis.text.y=element_blank(),
        axis.ticks=element_blank(),
        axis.title.x=element_blank(),
...
g1 / g2 + plot_annotation('Figure 4: Top 5 Employment Titles by HRH Expenditure, or by Individual Count', theme=theme(plot.title=element_text(size=10)))
...
detect_outlier <- function(x) {
  ...
  Quantile1 <- quantile(x, probs=.001)
  # calculate third quantile
  Quantile3 <- quantile(x, probs=.999)
...
ggplot(remuneration, 
  ...
  plot_annotation('Figure 5: Median Annual Remuneration for Top 10 Employment Titles', theme=theme(plot.title=element_text(size=16))) +
...
# Now plot the summary df
  ...
  plot_annotation('Figure 6: Prime Local Partner Support of HRH by Primary Program Area', theme=theme(plot.title=element_text(size=10))) +
...
## Additionally, calculate the % of HRH expenditures channeled through prime local partners
  ...
  Local_pct <- HRH_clean %>%
    ...
    mutate(is_indigenous_prime_partner = case_when(is_indigenous_prime_partner == "Y" ~"Yes",
                                                   is_indigenous_prime_partner == "N" ~ "No",
                                                   TRUE ~ is_indigenous_prime_partner)) %>%
    group_by(is_indigenous_prime_partner) %>%
    summarise(actual_annual_spend = sum(actual_annual_spend, na.rm = T)) %>%
    arrange(desc(actual_annual_spend)) %>%
    mutate(pct_of_total = round((actual_annual_spend / sum(actual_annual_spend))*100)) %>%
    filter(is_indigenous_prime_partner == "Local") %>%
    pull(pct_of_total)
  
  International_pct <- HRH_clean %>%
    filter(fiscal_year == 2024) %>%
    filter(funding_agency == "USAID") %>%
    filter(operating_unit == params$operating_unit) %>%
    select(fiscal_year, funding_agency, program, is_indigenous_prime_partner, actual_annual_spend) %>%
    ...
    arrange(desc(actual_annual_spend)) %>%
    mutate(pct_of_total = round((actual_annual_spend / sum(actual_annual_spend))*100)) %>%
    filter(is_indigenous_prime_partner == "International") %>%
    pull(pct_of_total)

```

Overall, `r Local_pct`% of HRH Expenditure is channeled through Prime Local Partners, while `r International_pct`% is channeled through Prime International Partners.

\pagebreak

## Work Location
Figure 7 shows the work location of USAID staff by HRH expenditure and individual count. Categories include above site, facility-based, community-based, or roving. Above site staff are defined as personnel that are not directly interacting or providing services to beneficiaries (e.g. technical assistance personnel or support staff based mostly in offices). Roving staff provide direct service delivery in multiple facilities. 
...
g2 <- ggplot(data = patchwork, mapping = aes(x = reorder(work_location, -actual_annual_spend), y = actual_annual_spend, fill = work_location)) +
  ...
  plot_annotation('Figure 7: HRH Expenditure and Individual Count by Work Location', theme=theme(plot.title=element_text(size=14))) +
...
#  mutate(employment_title_wrapped = str_wrap(employment_title, width = 10)) %>%
#  select(-employment_title) %>%
  mutate(work_location = "Community")

patchwork_facility_roving <- HRH_clean %>%
  filter(fiscal_year == 2024) %>%
  filter(funding_agency == "USAID") %>%
  filter(operating_unit == params$operating_unit) %>%
  filter(work_location == "Facility" | work_location == "Roving") %>%
  select(fiscal_year, work_location, employment_title, actual_annual_spend, individual_count) %>%
  group_by(employment_title) %>%
  summarise(actual_annual_spend = sum(actual_annual_spend, na.rm = T),
            individual_count = sum(individual_count, na.rm = T)) %>%
...
#  mutate(employment_title_wrapped = str_wrap(employment_title, width = 10)) %>%
#  select(-employment_title) %>%
  mutate(work_location = "Facility/Roving")

patchwork_aboveSite <- HRH_clean %>%
  filter(fiscal_year == 2024) %>%
  filter(funding_agency == "USAID") %>%
  filter(operating_unit == params$operating_unit) %>%
  filter(work_location == "Above Site") %>%
  select(fiscal_year, work_location, employment_title, actual_annual_spend, individual_count) %>%
  group_by(employment_title) %>%
  summarise(actual_annual_spend = sum(actual_annual_spend, na.rm = T),
            individual_count = sum(individual_count, na.rm = T)) %>%
...
#  mutate(employment_title_wrapped = str_wrap(employment_title, width = 10)) %>%
#  select(-employment_title) %>%
  mutate(work_location = "Above Site")


# Facility/Roving chart
g1 <- patchwork_facility_roving %>%
    ggplot(aes(x = reorder(employment_title, individual_count))) +
    geom_col(
  ...
  labs(title = stringr::str_glue("Facility or Roving Staff")) +
...
g2 <- patchwork_community %>%
  ...
  labs(title = stringr::str_glue("Community-based Staff")) +
...
g3 <- patchwork_aboveSite %>%
  ...
  labs(title = stringr::str_glue("Above Site Staff")) +
...
g1 / g2 / g3 + plot_annotation('Figure 8: Top Five Employment Titles for Facility/Roving, Community, and Above Site staff', theme=theme(plot.title=element_text(size=14))) 

```

\pagebreak
## Primary Beneficiaries for Community-based Staff
Figure 9 shows the breakdown of primary beneficiaries supported by Community-based staff by HRH Expenditure and Individual Count
...
g3 <- g2 + g1 + plot_annotation('Figure 9: Primary Beneficiaries of Community-based staff', theme=theme(plot.title=element_text(size=12)))
