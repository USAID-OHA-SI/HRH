...
HRH_data_orig <- read_excel("./1. Data/HRH_Structured_Datasets_Site_IM_FY21-22_not_redacted_20221216_withLP.xlsx") 
...
for (i in 1:length(OU_list)) {
  ...
  OU <- OU_list[i] # loop through each OU in the OU list
      ...
      wb <- loadWorkbook("./1. Data/HRH_QC_Reporting_Template_20221118_vF - Summary.xlsx")
  ...
  #    writeData(wb, sheet = 5, Breakdown_other, startCol = 2, startRow = 4, colNames = FALSE)
      ...
      wbName <- paste0("./4. Outputs/QC Reports/Summary only/HRH Data Quality Checks - Summary - ", OU, ".xlsx")
